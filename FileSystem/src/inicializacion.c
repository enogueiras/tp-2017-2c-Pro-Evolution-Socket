	...
	for (int i = 0; i < counter; i++) {
		...
		nodos[i].libre = config_get_int_value(restore_config,
				string_from_format("%s%s", tablaNodos->nombre_nodos[i],
						"Libre"));
		if (!(fileBitmap = fopen(
				string_from_format("%s%s%s%s", configFS->ruta_metadata,
...
void restablecerEstado() {
	...
	configFS->restore = false;
	t_config* restore_config = config_create(string_from_format("%s%s",configFS->ruta_metadata,"/nodos.bin"));
	fileDirectorios = fopen(string_from_format("%s%s", configFS->ruta_metadata,"/directorios.dat"), "wb+");
	...
	if (size_bytes== 0 || restore_config == NULL || !config_has_property(restore_config, "TAMANIO")){
		...
		ingresarDirectorios(string_from_format("%s%s", configFS->ruta_metadata, "/archivos/"));
	}
